...
/**
 * Polls {@code payments-stream} via a Redis Streams consumer group and hands each entry to {@link
 * PaymentProcessingService}. Runs its blocking read loop on a background thread so it never blocks
 * Spring Boot startup.
 */
@Component
public class PaymentStreamConsumer implements ApplicationRunner {
  ...
  /**
   * Redis Streams returns a {@code BUSYGROUP} error when the consumer group already exists. Spring
   * Data Redis wraps this as a {@link DataAccessException} (typically {@code RedisSystemException})
   * whose own message is a generic "Error in execution" — the actual {@code BUSYGROUP ...} text
   * only appears on the nested cause (e.g. {@code io.lettuce.core.RedisBusyException}). So the
  ...
  static boolean isBusyGroupError(Throwable ex) {
    ...
    return mostSpecificCause.getMessage() != null
        && mostSpecificCause.getMessage().contains("BUSYGROUP");
  }
  ...
  private void pollLoop() {
    ...
    while (true) {
      try {
        ...
        log.error(
            "Unexpected error reading from stream {}; will retry on next poll", streamKey, ex);
        try {
  ...
  private void handleRecord(MapRecord<String, Object, Object> record) {
    try {
      ...
      log.error(
          "Unexpected exception processing record {}; leaving unacked in PEL", record.getId(), ex);
    }
  }
}
