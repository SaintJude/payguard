...
/**
 * Stubbed downstream payment processor. Deliberately fails on the very first call for a given
 * paymentId and succeeds on every call after that, to simulate a transient downstream failure and
 * prove the retry path works. Attempt counts are in-memory only and reset on worker restart.
 */
@Component
